...
export class AllItemsComponent implements OnInit {
  ...
  getAllItems() {
    this.loading.start();
    this.authItemContributorService.getAuthenticatedAllItemsByShopId().pipe(takeUntil(this.ngUnsubscribe))
      .subscribe(result => {
        this.allItems = result.result;
        this.displayItems = result.result;
        this.sharedCategoryService.numberOfAllItems.next(this.allItems.length);
