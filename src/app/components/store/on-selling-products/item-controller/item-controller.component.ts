...
export class ItemControllerComponent implements OnInit {
  ...
  isModifyOnSellingItemModalOpened: boolean;
  moment = moment;
  removeMessage = '';
  removeSubmessage = '';
  columns = [];
  ...
  openRemovePermanentlyModal() {
    ...
    this.removeSubmessage = 'Caution: Item will be removed from the category!';
    this.isRemoveAllSelectedItemModalConfirmationOpened = true;
  }
  openModifySellingItemModalOpened() {
    this.isModifyOnSellingItemModalOpened = true;
    this.action = () => {
      this.sharedCategoryService.categoryRefresh.next({refresh: true, loading: false});
    }
  }
  checkSelectedItems() {
  ...
  markAsNew() {
    ...
    WsToastService.toastSubject.next({ content: 'Item(s) are marking as New...', type: 'default'});
    this.authOnSellingItemContributorService.markAsNew(this.editItems)
      .pipe(takeUntil(this.ngUnsubscribe))
      .subscribe(results => {
        this.sharedCategoryService.refreshCategories(() => {
          WsToastService.toastSubject.next({ content: "Item(s) are marked as New!", type: 'success' });
        })
      }, (err) => {
        WsToastService.toastSubject.next({ content: err.error, type: 'danger' });
      });
  ...
  unmarkNew() {
    this.previousEditedItems = [...this.editItems];
    WsToastService.toastSubject.next({ content: 'Item(s) are unmarking from New...', type: 'default'});
    this.authOnSellingItemContributorService.unmarkNew(this.editItems)
      .pipe(takeUntil(this.ngUnsubscribe))
      .subscribe(results => {
        this.sharedCategoryService.refreshCategories(() => {
          WsToastService.toastSubject.next({ content: "Item(s) are unmarked from New!", type: 'success' });
        })
      }, (err) => {
        WsToastService.toastSubject.next({ content: err.error, type: 'danger' });
      });
  ...
  removeItemsFromCategory() {
    ...
    observableForkJoin(editItems.map(item => {
      return this.authOnSellingCategoryContributorService.removeItemsFromCategory({
        items: editItems.map(x => x['_id']),
  ...
  removeItemsPermanantly() {
    ...
    this.previousEditedItems = [];
    this.loading.start();
    this.authOnSellingItemContributorService.removeItemsPermanantly({
      items: editItems.map(x => x['_id'])
    })
      .pipe(takeUntil(this.ngUnsubscribe))
      .subscribe(result => {
  ...
  getCategoriesByStoreId() {
    ...
    this.authOnSellingCategoryContributorService.getAuthenticatedCategoriesByStoreId()
