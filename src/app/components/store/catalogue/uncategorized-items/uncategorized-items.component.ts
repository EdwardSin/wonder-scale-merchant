...
export class UncategorizedItemsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    DocumentHelper.setWindowTitleWithWonderScale('Uncategorized - ' + store_name);
    this.loading.start();
  ...
  getUncategorizedItems(keyword = '', page = 1, order = 'alphabet', orderBy, isLoading = true) {
    ...
    this.getUncategorizedItemsSubscription = combineLatest(timer(500), this.authItemContributorService.getAuthenticatedUncategorizedItemCategoryByStoreId({ keyword, page, order, orderBy }))
