...
export class DiscountItemsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    DocumentHelper.setWindowTitleWithWonderScale('Discount - ' + store_name);
    this.loading.start();
  ...
  getAllDiscountItems(keyword = '', page = 1, order = 'alphabet', orderBy, isLoading = true) {
    ...
    this.getDiscountItemsSubscription = combineLatest(timer(500), this.authItemContributorService.getAuthenticatedDiscountItemsByStoreId({ keyword, page, order, orderBy }))
