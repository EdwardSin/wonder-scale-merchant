...
export class PublishedItemsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    DocumentHelper.setWindowTitleWithWonderScale('Published - ' + store_name);
    this.loading.start();
  ...
  getPublishedItems(keyword = '', page = 1, order = 'alphabet', orderBy, isLoading = true) {
    ...
    this.getPublishedItemsSubscription = combineLatest(timer(500),
      this.authItemContributorService.getAuthenticatedPublishedItemCategoryByStoreId({ keyword, page, order, orderBy }))
      .pipe(map(x => x[1]),
