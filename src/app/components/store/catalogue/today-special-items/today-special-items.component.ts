...
export class TodaySpecialItemsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    DocumentHelper.setWindowTitleWithWonderScale('Today Special - ' + store_name);
    this.loading.start();
  ...
  getTodaySpecialItems(keyword = '', page = 1, order = 'alphabet', orderBy, isLoading = true) {
    ...
    this.getTodaySpecialItemsSubscription = combineLatest(timer(500),
      this.authItemContributorService.getAuthenticatedTodaySpecialItemsByStoreId({ keyword, page, order, orderBy }))
      .pipe(map(x => x[1]),
